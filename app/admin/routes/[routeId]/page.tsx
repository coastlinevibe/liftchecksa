...
import {
  ...
  removeRouteSeatPassenger,
  reviewRouteSeatCancellation,
...
export default async function AdminRouteDetailPage({
  ...
  searchParams?: Promise<{ chat_error?: string; offer_error?: string; offer_success?: string; admin_status?: string; admin_message?: string }>;
}) {
  ...
  const updatePhoneCallVerifiedAction = async (formData: FormData) => {
    'use server';
    const result = await updateDriverRouteApplicationPhoneCallVerified({
    ...
    if ('error' in result) {
      redirect(`/admin/routes/${routeId}?admin_status=error&admin_message=${encodeURIComponent(result.error || 'Unable to save verification')}`);
    }

    redirect(`/admin/routes/${routeId}?admin_status=success&admin_message=${encodeURIComponent('Phone verification saved')}`);
  };

  const updateApplicationDecisionAction = async (formData: FormData) => {
    'use server';
    const decision = String(formData.get('decision') || '') as 'approved' | 'rejected';
    const result = await updateDriverRouteApplicationDecision({
      assignmentId: String(formData.get('assignmentId') || ''),
      routeId,
      decision,
    });
    ...
    if ('error' in result) {
      redirect(`/admin/routes/${routeId}?admin_status=error&admin_message=${encodeURIComponent(result.error || 'Unable to update driver application')}`);
    }

    const successMessage = decision === 'approved' ? 'Driver application approved' : 'Driver application declined';
    redirect(`/admin/routes/${routeId}?admin_status=success&admin_message=${encodeURIComponent(successMessage)}`);
  };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto grid max-w-6xl gap-4 px-4 py-4 lg:grid-cols-[1.4fr_1fr]">
        <div className="space-y-4">
          {resolvedSearchParams?.admin_status === 'success' && resolvedSearchParams?.admin_message ? (
            <section className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
              {resolvedSearchParams.admin_message}
            </section>
          ) : null}

          {resolvedSearchParams?.admin_status === 'error' && resolvedSearchParams?.admin_message ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="text-sm text-slate-600">
                        ...
                        {stop.is_start ? (
                          <span className="ml-2 inline-flex rounded-full bg-emerald-100 px-2 py-0.5 text-[13px] font-semibold text-emerald-700">
                            Start
                          </span>
                        ) : null}
                        {stop.is_end ? (
                          <span className="ml-2 inline-flex rounded-full bg-sky-100 px-2 py-0.5 text-[13px] font-semibold text-sky-700">
                            Destination
                          </span>
                        ) : null}
                      </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="space-y-3">
              {pendingApplications.map((assignment) => {
                ...
                return (
                  <details key={assignment.id} className="group rounded-lg border border-slate-200 bg-slate-50 p-3">
                    <summary className="cursor-pointer list-none [&::-webkit-details-marker]:hidden">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">{applicationHeader}</div>
                        <div className="flex shrink-0 items-center gap-1 rounded-full border border-blue-200 bg-blue-50 px-2 py-1 text-[11px] font-semibold text-blue-700">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="space-y-3">
              {requests.map((request) => {
                ...
                return (
                  <details key={request.id} className="group rounded-lg border border-slate-200 bg-slate-50 p-3">
                    <summary className="cursor-pointer list-none [&::-webkit-details-marker]:hidden">
                      <div className="flex items-start justify-between gap-3">
